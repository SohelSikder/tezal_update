...
if (!function_exists('Category')) {
    function Category()
    {
        return Category::where('parent_id',0)->where('status',1)->where('is_featured',1)->get();
    }
}
...
if (!function_exists('CategoryBanner')) {
...
function special_characters_replace($data) {
    $string = $data;
    $pattern = '/[^a-zA-Z0-9\s]/';
    $replacement = "_";
...
if (!function_exists('productSizeCount')) {
    function productSizeCount($id)
    {
        $size = Size::with('products')->where('id', $id)->first();
        $count = $size->products->count('id');
        return $count;
    }
}
