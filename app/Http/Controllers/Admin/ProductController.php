...
class ProductController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->editColumn('PrimaryImage', function ($data) {
                    ...
                    return '<img src=' . $url . ' alt='.$data->Primary_Image.' border="0" width="50" class="img-rounded" align="center" />';
                })
                ...
                ->editColumn('en_About', function ($data) {
                    return $data->en_About;
                })
    ...
    public function productCreate()
    {
        ...
        $data['category'] = Category::where('category_type', 0 )->get();
    ...
    public function physicalProductCreate()
    {
        ...
        $data['product'] = Product::get();
        $data['categories'] = Category::where('category_type', 0 )->get();
        $data['sub_categories'] = Category::where('category_type', 1 )->get();
        $data['sub_sub_categories'] = Category::where('category_type', 2 )->get();
        $data['tags'] = ProductTagList::get();
        $data['item_tags'] = ItemTag::get();
        return view('admin.pages.product.physical', $data);
    }

    public function physicalProductCsvUpload(){
        $data['title'] = __('Product Bulk Upload');
        $data['product'] = Product::get();
        $data['categories'] = Category::where('category_type', 0 )->get();
        $data['sub_categories'] = Category::where('category_type', 1 )->get();
        $data['sub_sub_categories'] = Category::where('category_type', 2 )->get();
        $data['tags'] = ProductTagList::get();
        $data['item_tags'] = ItemTag::get();
        return view('admin.pages.product.csv_upload', $data);
    }
    public function physicalProductImport(Request $request){
        $request->validate([
            'en_category_name' => 'required',
            'import_file' => 'required',
        ]);
        Excel::import(new ProductImport($request), request()->file('import_file'), \Maatwebsite\Excel\Excel::CSV);
    ...
    public function sub_cat_data(Request $request){
        ...
        // Retrieve subcategories based on the provided categoryId
        $subcategories = Category::where('parent_id', $categoryId)->get();

        return response()->json(['subcategories' => $subcategories]);
    }
    public function sub_sub_cat_data (Request $request){
        // dd('ok');
    ...
    {
        // dd($data['physicalProductAdd($data)']);
        ...
        $product = Product::create([
            ...
            'sub_sub_category_id' => $data['sub_sub_category'] ?? null,
    ...
    public function productDelete($id)
    {
        ...
        if ($delete) {
            if($delete->Primary_Image){
                if(file_exists(ProductImage().$delete->Primary_Image)){
                    unlink(ProductImage().$delete->Primary_Image);
                }
            }
            if($delete->Primary_Image){
                if(file_exists(ProductImage().$delete->Primary_Image)){
    ...
    public function physicalProductUpdate($data, $product)
    {
        ...
        $update = $product->update([
            ...
            'en_Product_Slug' => Str::slug($data['en_product_name']),
            ...
            //'fr_Quantity' => is_null($data['fr_qty']) ? $product->Quantity : $data['fr_qty'],
