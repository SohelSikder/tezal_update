...
class UserController extends Controller
{
    ...
    public function addressList(Request $request){
        if ($request->ajax()) {
            // $data = Address::all();
            // SELECT `id`, `name`, `phone`, `address_type`, `city`, `area`, `created_at`, `updated_at` FROM `addresses` WHERE 1
            // SELECT `id`, `name`, `district_id`, `is_active`, `created_at`, `updated_at` FROM `areas` WHERE 1
            // SELECT `id`, `name`, `is_active`, `created_at`, `updated_at` FROM `districts` WHERE 1
            $data = DB::table('addresses')
            ->join('areas','areas.id','addresses.area')
            ->join('districts','districts.id','addresses.city')
            ->select('addresses.id','addresses.name','addresses.phone','addresses.address_type','areas.name as area', 'districts.name as city')->get();
            return DataTables::of($data)
                ->addIndexColumn()
                ->addColumn('orders', function ($data) {
                    return orderCountuser($data->id);
